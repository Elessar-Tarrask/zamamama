...
- [ ] **M1 — первый деплой** *(ждёт: проект Firebase + ключ Wazzup)*
- [ ] **Живая проверка M2–M4** с реальными ключами (Azure OpenAI, календарь) и фикс найденных багов
- [ ] **M5** — полировка после запуска: алерты об ошибках, TTL-политики Firestore, финальная приёмка
